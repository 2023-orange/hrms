package com.sunten.hrms.re.domain;

import com.baomidou.mybatisplus.annotation.IdType;
import com.baomidou.mybatisplus.annotation.TableId;
import com.sunten.hrms.base.BaseEntity;
import lombok.Data;
import lombok.EqualsAndHashCode;
import lombok.ToString;
import lombok.experimental.Accessors;
import javax.validation.constraints.NotBlank;
import javax.validation.constraints.NotNull;
import java.time.LocalDateTime;
import java.util.List;

/**
 * <p>
 * 用人需求岗位子表
 * </p>
 *
 * @author liangjw
 * @since 2021-04-23
 */
@Data
@EqualsAndHashCode(callSuper = true)
@ToString(callSuper = true)
@Accessors(chain = true)
public class ReDemandJob extends BaseEntity {

    private static final long serialVersionUID = 1L;

    /**
     * 用人需求id
     */
    @NotNull
    private Long demandId;

    /**
     * 编号
     */
    private String demandCode;

    /**
     * 剩余人数
     */
    private String remainingPersonnel;
    /**
     * 岗位名称
     */
    @NotBlank
    private String jobName;

    private Long jobId;

    /**
     * 岗位人数
     */
    @NotNull
    private Integer quantity;

    /**
     * 岗位素质要求
     */
    private String baseCondition;

    /**
     * 是否特殊岗位
     */
    @NotNull
    private Boolean specialFlag;

    /**
     * 特殊入职体检项目
     */
    private String specialMidecal;

    /**
     * 是否需要岗位说明书
     */
    @NotNull
    private Boolean needFlag;

    /**
     * 生效标记
     */
    @NotNull
    private Boolean enabledFlag;

    private Boolean realType; // 用于前台控制删除是否请求数据库

    @TableId(value = "id", type = IdType.AUTO)
    @NotNull(groups = Update.class)
    private Long id;

    private String realFlag;

    private Boolean showBox;

    private List<ReDemandJobDescribes> demandJobDescribesList;

    // 根据岗位id显示的岗位名称
    private String jobNameById;

    // 危险因素
    private String riskFactors;

    // 班次信息
    private String teamMes;

    // 岗位同期当年人数
    private Integer jobFirstCount;

    // 岗位同期去年人数
    private Integer jobSecondCount;

    // 岗位同期前年人数
    private Integer jobThirdCount;

    // 在占用的数量
    private Integer inUsedQuantity;

    // 通过的数量
    private Integer passQuantity;

    // 岗位类别
    private String jobClass;

    // 记录手动开放的用户id
    private Long demandOpenBy;

    // 记录手动开放的操作时间
    private LocalDateTime demandOpenTime;

    // 更新手动开放时的InUsedPass
    private Integer newInUsedQuantity;

    // 用于区分是否在审批前提交
    private Boolean addBeforeApprovalFlag;

    // 是否需要检测
    private Boolean needCheckFlag;

    // 用于检测需确认数量与确认数量是否一致
    private Boolean checkAfterUpdateFlag;

    // 岗位当前人数
    private Integer jobCurrentCount;

    // 同意人数
    private Integer agreeNum;

}
