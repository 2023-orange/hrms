...
public class DomainEqualsUtil {
    ...
    public static boolean domainEquals(Object source, Object target) {
        ...
        log.debug("THE EQUALS RESULT IS " + rv);
        return rv;
    }
    ...
    public static Object getClassValue(Object obj, String fieldName) {
        ...
        try {
            ...
            // log.debug("取方法出错！" + e.toString());
        }
        return null;
    }

    /**
     * 根据cloumS的字段名称取domainOne，domainTwo值作比较，返回不等的结果
     * @param columnS
     * @param domainOne
     * @param domainTwo
     * @return
     */
    public static List<DomainEqualsResult> domainEquals(String columnS, Object domainOne, Object domainTwo){
        String[] equalsCheckSL = columnS.split(",");
        List<DomainEqualsResult> domainEqualsResultList = new ArrayList<>();
        for(int i=0;i<equalsCheckSL.length;i++){
            String cloumnName = equalsCheckSL[i];
            String valueOne = DomainEqualsUtil.getClassValue(domainOne,cloumnName) == null? "" :DomainEqualsUtil.getClassValue(domainOne,cloumnName).toString();
            String valueTwo = DomainEqualsUtil.getClassValue(domainTwo,cloumnName) == null? "" :DomainEqualsUtil.getClassValue(domainTwo,cloumnName).toString();
            if(!valueOne.equals(valueTwo)){
                DomainEqualsResult domainEqualsResult = new DomainEqualsResult();
                domainEqualsResult.setCloumnName(cloumnName);
                domainEqualsResult.setValueOne(valueOne);
                domainEqualsResult.setValueTwo(valueTwo);
                domainEqualsResultList.add(domainEqualsResult);
            }
        }
        return domainEqualsResultList;
    }
}
